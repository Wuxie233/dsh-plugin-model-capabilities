...
/** Request modalities the llm-pi-ai schema accepts ('video' is the DSH-side superset modality). */
const KNOWN_MODALITIES = new Set(['text', 'image', 'video'])
 ...
 * @property {{thinkingFormat?: string, supportsReasoningEffort?: boolean, supportsDeveloperRole?: boolean}} [compat]
 ...
 * protocol, so filling it there would break the whole write. An existing
 * compat object that omitted `supportsDeveloperRole` reports that key so
 * fillMissing can merge it without replacing the rest of the object.
...
export function missingFields(model, routeApi) {
  ...
  if (routeApi === undefined || routeApi === 'openai-completions') {
    if (model.compat === undefined || model.compat === null) {
      missing.push('compat')
    } else if (isPlainObject(model.compat) && model.compat.supportsDeveloperRole === undefined) {
      missing.push('compat.supportsDeveloperRole')
    }
  }
  return missing
}
 ...
 * property: user-written values always win over source data. An existing
 * `compat` object that omitted `supportsDeveloperRole` is merged in place:
 * source facts win when they name the switch, otherwise `thinkingFormat: zai`
 * (on the source or the current entry) fills `false`.
 * @param {Record<string, unknown>} model - raw user-layer model entry.
...
export function fillMissing(model, caps, wantMissing) {
  ...
  if (wantMissing.includes('compat') || wantMissing.includes('compat.supportsDeveloperRole')) {
    const sourceCompat = isPlainObject(caps.compat) ? caps.compat : {}
    const currentCompat = isPlainObject(next.compat) ? next.compat : {}
    if (currentCompat.supportsDeveloperRole === undefined) {
      const role = sourceCompat.supportsDeveloperRole
      const zai = sourceCompat.thinkingFormat === 'zai' || currentCompat.thinkingFormat === 'zai'
      const value = role !== undefined ? role : (zai ? false : undefined)
      if (value !== undefined) {
        if (next === model) next = { ...model }
        next.compat = { ...currentCompat, supportsDeveloperRole: value }
        filled.push('compat.supportsDeveloperRole')
      }
    }
  }
...
export function piAiEntryToCapabilities(entry, sourceLabel) {
  ...
  if (entry.api === 'openai-completions' && isPlainObject(entry.compat)) {
    ...
    if (typeof entry.compat.supportsDeveloperRole === 'boolean') {
      compat.supportsDeveloperRole = entry.compat.supportsDeveloperRole
    }
