 ...
 * hot-reload. Fields already present in settings are never touched. An existing
      ...
      try {
        ...
        for (const [route, sectionRaw] of Object.entries(providers)) {
          ...
          const nextModels = await Promise.all(models.map(async model => {
            ...
            if (caps === undefined && !miss.includes('compat.supportsDeveloperRole')) return model
            const source = caps ?? { source: 'implied-zai' }
            const { model: next, filled } = fillMissing(model, source, miss)
            if (filled.length === 0) return model
            routeChanged = true
            log.info(`model-capabilities: ${route}/${model.id} <- ${source.source}: ${filled.join(', ')}`)
            return next
          }))
          if (routeChanged) {
